...
Generates a small set of minimal SVG-based wallpapers on first run so users
have attractive options out of the box (no network needed). The SVGs are
written to disk and served as-is — browsers render SVG via <img> just fine.
"""
...
def get_preset_dir(assets_root: Path) -> Path:
    """Directory where preset SVGs are cached."""
    return assets_root / "wallpapers"


def ensure_presets(assets_root: Path) -> list[dict]:
    """Write preset SVGs to disk on first run; return their info list."""
