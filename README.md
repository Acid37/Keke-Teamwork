...
Keke Teamwork 当前处于 **v0.3 规划阶段**（v0.2 已收尾）。核心方向是构建**角色驱动的多 Agent 工作流**（Plan → Code → Review 闭环），而非单纯的工具平台。
...
v0.3 建设中的能力：per-agent 细粒度权限、内置角色 Agent 定义（planner/coder/reviewer）、事件持久化、基础模块测试补全。
...
| 阶段 | 重点 |
|---|---|
| **v0.3** — 角色体系基础 | per-agent 细粒度权限、内置角色（planner/coder/reviewer）、事件持久化、测试补全、Setup Wizard |
