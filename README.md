...
- **Agent 管理**：可创建和编辑任意自定义 Agent 定义，包括名称、角色、模型、温度、工具列表、最大工具轮次和系统提示词。
...
关闭 Solo 后，在存在只读 Agent 时会先运行只读并行研究，把合并摘要注入 main Agent 上下文。main Agent 也可以通过 `delegate_agent` 把写入任务安全 handoff 给有写工具的 Agent。分流基于工具分类自动判断，不依赖角色名。
