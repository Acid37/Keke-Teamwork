...
Keke Teamwork 是一个本地多 Agent 协作工作台，当前处于 **v0.4** 阶段，工作流引擎正在开发中。

两层架构：

- **工作流层**：定义"谁在什么时候做什么"（Plan -> Code -> Review 闭环，v0.4 落地）。
  v0.4 工作流引擎（WorkflowRunner）以状态机方式驱动阶段流转：接收用户需求后，依次触发 Planner 拆解任务、Coder 逐任务编码、Reviewer 审查代码，并在阶段间传递结构化产出物（TaskList -> DiffSet -> ReviewReport），支持用户确认计划和审查反馈重试。
...
backend/
  ...
  types.py              # 共享数据类型（AgentDefinition / Session / Phase 等）
  llm/
  ...
  workflow/             # v0.4 工作流引擎
    __init__.py         # 模块入口，导出 WorkflowRunner 及核心类型
    engine.py           # WorkflowRunner 状态机主循环
    types.py            # 阶段间数据结构（TaskList / DiffSet / ReviewReport / WorkflowState）及阶段守卫
...
**v0.4 工作流引擎路径（WorkflowRunner 主循环）：**

当启用了工作流模式的会话接收到用户消息时，Orchestrator 将执行权转交给 WorkflowRunner：

```
WorkflowRunner.execute(session, user_text, broadcast)
  |
  |- Phase.INIT -> PLANNING：广播"正在分析需求..."
  |
  |- Phase.PLANNING -> _run_planner()：调用 Planner Agent 产出 TaskList
  |    - 产出物：TaskList（overview + 有序子任务 + 风险 + 预估工时）
  |    - 广播 plan 到前端，等待用户确认 -> PLAN_REVIEW
  |
  |- Phase.PLAN_REVIEW -> 用户确认后进入 CODING
  |
  |- Phase.CODING -> _run_coder()：逐子任务执行
  |    - 取当前子任务（SubTask），调用 Coder Agent 编码
  |    - 产出物：DiffSet（变更文件 + diff 文本 + 摘要）
  |    - 自动进入 CODE_REVIEW
  |
  |- Phase.CODE_REVIEW
  |    - Auto Review 模式 -> 自动进入 REVIEWING
  |    - 手动模式 -> 等待用户触发审查
  |
  |- Phase.REVIEWING -> _run_reviewer()：审查当前 DiffSet
  |    - 产出物：ReviewReport（逐文件审查 + 总体判定 + 是否重试）
  |    - 需要重试 -> 进入 FEEDBACK，注入审查反馈到 coder_guidance_queue
  |    - 审查通过 -> 推进到下一子任务，继续 CODING
  |
  |- Phase.FEEDBACK -> 用户确认后回到 CODING 重试当前任务
  |
  |- Phase.COMPLETED -> 广播完成信息，工作流结束
  |
  - Phase.ERROR -> 等待用户介入恢复
```

WorkflowRunner 不关心 Agent 内部如何执行（tool-calling 循环仍走现有 Agent.run()），只关心阶段产出物和转换条件。
...
`role` 字段当前只是显示/组织标签，不触发模型回退或工具分流。v0.4 将使其语义化，与 toolset 共同决定 Agent 行为。
...
| 模块 | 职责 |
|---|---|
| `AgentOrchestrator` | 接管用户消息执行，协调并行研究、handoff、标题生成；v0.4 中负责在启用工作流模式时转交执行权给 WorkflowRunner |
| `WorkflowRunner` | v0.4 工作流状态机主循环，驱动 Plan -> Code -> Review 阶段流转，管理阶段间数据传递 |
...
| `normal` | 普通命令（`git add`、`npm install` 等） | 非 YOLO 模式需审批，YOLO 模式放行 |
...
首次启动时写入 `agents.json`，内置 4 个默认 Agent，形成基本角色矩阵：
