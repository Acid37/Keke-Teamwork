...
backend/
  workflow/
    ...
    triggers.py           # 阶段转换触发条件 + 自动调度逻辑（待实现）
    context.py            # 阶段间上下文管理（注入/剪枝策略）（待实现）
```
...
| `backend/orchestrator.py` | 入口改为委托到 WorkflowRunner；保留单 Agent 执行逻辑 | 部分完成 |
| `backend/prompt_builder.py` | 阶段感知提示词（coder 收到 TaskList、reviewer 收到 DiffSet） | 部分完成 |
...
| **Step 4** | 产出物解析：`parse_task_list()` / `parse_review_report()` | 0.5d | 正例 + 反例 + 启发式回退 | 待开始 |
| **Step 5** | 自动触发串联：plan → code → review 完整链路 | 1d | 集成测试（Mock LLM，验证触发链路） | 待开始 |
| **Step 6** | 用户命令处理：确认/跳过/修改/中断 | 0.5d | 命令队列测试 | 待开始 |
| **Step 7** | 阶段状态持久化 + 恢复 | 0.5d | `WorkflowState` 序列化 + 恢复测试 | 待开始 |
