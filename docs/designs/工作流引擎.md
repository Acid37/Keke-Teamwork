...
stateDiagram-v2
    ...
    CODE_REVIEW --> CODING : 用户 undo，回退上一个已完成任务
    FEEDBACK --> CODING : 用户 undo，回退上一个已完成任务
...
class WorkflowState:
    ...
    retry_count: int = 0               # 当前任务重试次数（上限 MAX_RETRIES_PER_TASK=3）
    total_files_changed: int = 0       # 累计文件变更数
...
| `CODE_REVIEW` | 跳过审查、手动触发审查、直接修改代码、**撤销上一任务（undo）** |
| `FEEDBACK` | 接受有风险代码、修改计划、手动修复、**撤销上一任务（undo）** |
...
- `handle_user_command()` 中每次命令处理后（approve_plan、reject_plan、start_review、skip_review、retry、skip_task、undo、abort、resume）
...
## 9. CLI 实时反馈与 files.changed 广播

### 9.1 files.changed 事件

`orchestrator.run_workflow_agent()` 在 agent 执行完成后，若 staging 有文件变更，自动广播 `files.changed` 事件：
...
CLI 广播闭包接收 `files.changed` 后，使用 `format_diff()` 和 `format_file_change()` 渲染：
...
agent 启动时显示旋转动画（`cli_display.Spinner`），在以下事件时自动停止：
- 阶段切换（`agent.status`）
- 工具调用（`tool.call`）
- 文本输出（`agent.text`）
- agent 完成（`agent.completed`）
- 错误（`error`）
...
`handle_user_command("undo")` 在 CODE_REVIEW / FEEDBACK 阶段可用：

1. 弹出 `completed_tasks` 最后一个任务 ID
2. `task_list.current_task_index` 回退 1
3. 重置回退任务状态为 `pending`
4. 清空 `current_diff_set`、`last_review_report`、`retry_count`、`coder_guidance_queue`
5. 阶段回到 `CODING`，重新编码
