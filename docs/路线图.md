...
| **v0.3** | 角色体系基础：per-agent 权限 + 内置角色 + 测试补全 | 已完成 |
...
- [x] **Phase 扩展**：Phase 扩展为 PLANNING → CODING → REVIEWING，阶段转换规则 + 进入/退出守卫
- [x] **WorkflowRunner 骨架**：实现 WorkflowRunner 基础框架，阶段编排与调度
- [x] **阶段感知提示词**：各阶段 Agent 提示词差异化，阶段上下文注入
- [x] **阶段间数据传递**：plan 产出 TaskList 结构体，coder 产出 DiffSet，reviewer 产出 ReviewReport
