...
- [ ] **per-agent 工具权限策略**：从二值（能写/不能写）升级为 per-agent 工具白名单 + 路径约束 + 命令风险预算
- [ ] **内置角色 Agent 定义**：planner / coder / reviewer 三个默认 Agent，各有差异化 toolset + system prompt
- [ ] **role 字段语义化**：`role` 不再是纯标签，开始参与分流判断（与 toolset 共同决定行为）
- [ ] **基础模块测试补全**：`SessionStore`、`FileStagingArea`、`EditTool`、`PermissionManager` 独立测试
- [ ] **事件持久化**：research/handoff/timeline 事件存入 session，刷新不丢失
- [ ] **Setup Wizard**：首次启动引导配置 API key、模型、默认角色
